...
bool ULMAWeaponComponent::CanReload() const
{
	...
	return !AnimReloading && Weapon && !Weapon->IsCurrentClipFull() && Character && !Character->GetSprinting();
}

void ULMAWeaponComponent::StopReload()
{
	const auto Character = Cast<ALMADefaultCharacter>(GetOwner());
	if (Character && !Character->GetSprinting())
	{
...
{
	const auto Character = Cast<ALMADefaultCharacter>(GetOwner());
	if (Weapon && CanReload() && Character && !Character->GetSprinting())
	{
		Reload();
	}
}
